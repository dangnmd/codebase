...
# 	from cpplib_osx import xtea_encrypt, xtea_decrypt, garena_xtea_encrypt, garena_xtea_decrypt	# pylint: disable=unused-import
# else:
# 	from cpplib import xtea_encrypt, xtea_decrypt, garena_xtea_encrypt, garena_xtea_decrypt	# pylint: disable=unused-import
...
def encrypt_secure_token(key, data):
	# provide confidentality and integrity
	nonce = os.urandom(AES_GCM_NONCE_SIZE)
	return AESGCM(key).encrypt(nonce, data, None).hex() + '.' + nonce.hex()

def decrypt_secure_token(key, token):
	try:
		parts = token.rsplit('.', 1)
		assert len(parts) == 2
		nonce = binascii.unhexlify(parts[1])
		encrypted = binascii.unhexlify(parts[0])
...
_percent_encode_unreserved = string.ascii_letters + string.digits + '_.-~'
_percent_encode_map = dict([(chr(c), chr(c) if chr(c) in _percent_encode_unreserved else '%%%02X' % c) for c in range(256)])

def percent_encode(s):
	"""
	quote('abc def') -> 'abc%20def'
	Implementation of RFC3986
	unicode is utf-8 encoded before percent encoded
	"""
	if s is None:
		return None
	if isinstance(s, str):
	...
	return ''.join([_percent_encode_map[b] for b in s])
...
def md5(plain_text):
	...
	return hashlib.md5(to_bytes(plain_text)).hexdigest()
...
def xtea_cbc_encrypt(data, key, endian='<'):
	...
	cipher = []
	block_cipher = 0
	for i in range(0, size, XTEA_BLOCK_SIZE):
...
def aes_cbc_encrypt(data, key):
	from .utils import to_bytes
	return AES.new(to_bytes(key), AES.MODE_CBC, to_bytes(AES_CBC_IV)).encrypt(data)

def aes_cbc_decrypt(data, key):
	try:
		from .utils import to_bytes
		return AES.new(to_bytes(key), AES.MODE_CBC, to_bytes(AES_CBC_IV)).decrypt(data)
	except:
		return None
...
def garena_xtea_unpadding(data):
	...
	for i in range(0, len(data) - XTEA_BLOCK_SIZE, XTEA_BLOCK_SIZE):
		qword = struct.unpack_from('<Q', data, i)[0]
		check_sum -= qword
