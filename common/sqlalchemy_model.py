...
def init_db(config, debug=False):
	...
	for key in config:
		value = config[key]
		conn_uri = '%s://%s:%s@%s:%s/%s' % (value['type'], urllib.parse.quote(value['user'], safe=''), urllib.parse.quote(value['password'], safe=''),
			urllib.parse.quote(value['host'], safe=''), value.get('port', 3306), urllib.parse.quote(value['db'], safe=''))
		if 'charset' in value:
			conn_uri += '?charset=' + urllib.parse.quote(value['charset'], safe='')
