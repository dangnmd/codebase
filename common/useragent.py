...
class UserAgent(object):
	...
	_pattern = re.compile(r'Garena(?P<product>\w+)\s*\/\s*(?P<version>\d+(?:\.\d+)*)\s*\(\s*(?P<device>(?:[^;()]|\([^;()]*\))*?)\s*;\s*(?P<os>[\w\.\-\s]*[\w\.\-])\s*;\s*(?P<lang>[\w\-]+)\s*;\s*(?:(?P<region>[\w]{2})\s*)(?:;\s*(?P<extra>(?:[\w\.\-\s]*[\w\.\-])?)\s*;?\s*)?\)')	# pylint: disable=line-too-long
	...
	def __init__(self, ua_string):
		...
		if s:
			...
			self.version = list(map(int, self._data['version'].split('.')))
		else:
			self._data = {}
