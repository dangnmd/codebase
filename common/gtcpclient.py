...
class GTcpClient(object):
	...
	def __init__(self, address, port, timeout=10, retry=True, on_connect=None, on_disconnect=None):
	...
	def send(self, request):
		...
		try:
			...
			log.warn('tcp_send_fail|error=send_fail_will_retry,address=%s,port=%u,retry=0,request=%s,ex=%s',
				self._address, self._port, request.encode('hex'), ex)
			if not self._connect():
		...
		try:
			...
			if isinstance(ex, socket.timeout):
				log.exception('tcp_request_fail|error=recv_length_timeout,address=%s,port=%u,retry=0,request=%s',
					self._address, self._port, request.encode('hex'))
				return None
	...
	def _on_connect(self):
		if self._on_connect_callback is not None:
			retry = self._retry
			self._retry = False	# disable retry during on_connect callback to prevent recursion
	...
	def _on_disconnect(self):
		if self._on_disconnect_callback is not None:
			try:
				...
				log.exception('tcp_client_on_disconnect_exception|address=%s,port=%s', self._address, self._port)
