class EnumBaseMeta(type):
	def __new__(mcs, clsname, bases, classdict):
		...
		d.update((n, v) for n, v in list(classdict.items()) if not n.startswith('__') and not callable(v) and not isinstance(v, (classmethod, staticmethod, dict)))
		type.__setattr__(mcs, 'NAME_TO_VALUE', d)
		type.__setattr__(mcs, 'VALUE_TO_NAME', dict((v, k) for k, v in list(d.items())))
		if d:
...
class EnumBase(object, metaclass=EnumBaseMeta):
	"""
		usage:
