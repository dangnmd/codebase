...
class BaseCache(object):
	...
	def hmget(self, name, keys):
		...
		:param name: the key of hash to be looked up.
		:param keys: a list of values need to look up
		"""
		raise NotImplementedError()

	def hmset(self, key, mapping):
		"""Add multiple values to the hash in cache (overwrites value, if key already
		exists in the cache).
		:param key: the key of hash  to be looked up.
		:param mapping: the data dict need to save into hash
		"""
		raise NotImplementedError()
	...
	def hincrby(self, key, field, delta=1):
		"""Increments the value of a key field by `delta`.  If the key, field does
		not yet exist it is initialized with `delta`.
		:param key: the key of hash.
		:param field: the field in the key of hash.
	...
	def georadius(self, key, longitude, latitude, radius, distance_unit='m', with_dist=False, with_coord=False, with_hash=False, count=None, sort=None, store=None, store_dist=None):
		"""Return the members of specified membersof the geospatial index that distance with (longitude, latitude) less than radius.

		:param key: the key of sorted set
		:param longitude: double
		:param latitude: double
		:param radius: double
		:param distance_unit: value in ['m', 'km', 'ft', 'mi']
		:param with_dist: instruct the server to reply
		:param with_coord
		:param with_hash
		:param count: limit response
		:param sort: value in ['ASC', 'DESC']
		:param store
		:param store_dist
		:returns: members
		:rtype: list
...
class MemoryCache(BaseCache):
	...
	def _trim_data(self):
		...
		if self._last_trim_time + self._trim_interval < now:
			trim_keys = []
			for key, (timeout, _value) in self._data.items():
...
class MemcachedCache(BaseCache):
	...
	def delete_many(self, keys, noreply=False):
		...
		for key in keys:
			...
			if _test_memcached_key(key):
				new_keys.append(key)
		return self._client.delete_multi(new_keys) != 0
...
class RedisCache(BaseCache):
	...
	def dump_object(self, value):
		...
		return b'!' + pickle.dumps(value, protocol=self._pickle_protocol)
	...
	# 	if not pattern:
	# 		return False
	# 	keys = self.raw_client.keys(self._normalize_key(pattern))
	# 	if not keys:
	# 		return False
	# 	self._client.delete(*keys)
	# 	return keys

	def delete_by_patterns(self, patterns):
		if not patterns:
			return []
		keys = []
		for pattern in patterns:
			_keys = self.raw_client.keys(self._normalize_key(pattern))
			keys.extend(_keys)
		if not keys:
			return []
		self._client.delete(*keys)
	...
	def hmget(self, name, keys, *args):
		return self._client.hmget(name, [self._normalize_key(k) for k in keys], *args)

	def hmset(self, key, mapping):
		self._client.hmset(self._normalize_key(key), mapping)
		return True
	...
	def hincrby(self, key, field, delta=1):
		return self._client.hincrby(self._normalize_key(key), field, delta)
	...
	def georadius(self, key, longitude, latitude, radius, distance_unit='m', with_dist=False, with_coord=False, with_hash=False, count=None, sort=None, store=None, store_dist=None):
		return self._client.georadius(self._normalize_key(key), longitude, latitude, radius, distance_unit, with_dist, with_coord, with_hash, count, sort, store, store_dist)

	# def get_keys_by_pattern(self, pattern):
	# 	keys = self.raw_client.keys(self._normalize_key(pattern))
	# 	return [self._remove_key_prefix(key) for key in keys]

class CodisCache(RedisCache):
	"""Same cache client as RedisCache, but disable transactions in set_many for Codis to work normally.
	"""
...
class SsdbCache(BaseCache):
	...
	def hmget(self, name, keys):
		...
		return self._client.hmset(name, keys)
...
class ReplicationCache(BaseCache):
	...
	def hmget(self, name, keys, *args):
		...
		if client is None:
			return None
		return client.hmget(name, keys, *args)

	def hmset(self, key, mapping):
		result = True
		for client in self._client:
			if not client.hmset(key, mapping):
				return False
		return result
	...
	def hincrby(self, key, field, delta=1):
		result = True
		for client in self._client:
			if not client.hincrby(key, field, delta):
				result = False
		return result
	...
	def georadius(self, key, longitude, latitude, radius, distance_unit='m', with_dist=False, with_coord=False, with_hash=False, count=None, sort=None, store=None, store_dist=None):
		...
		if client is None:
			return None
		return client.georadius(key, longitude, latitude, radius, distance_unit, with_dist, with_coord, with_hash, count, sort, store, store_dist)

class DistributionCache(BaseCache):
	"""A cache distributior based on cache key.

	:param method: support following methods. default is conhash
	...
	def _get_client_by_key_hash_mod(self, key):
		client_id = crypt.crc32(key) % self._factor
		return self._client.get(client_id)
	...
	def hmget(self, name, keys, *args):
		client = self.get_client_by_key(name)
		if client is None:
			return False
		return client.hmget(name, keys, *args)
...
class MultilayerCache(BaseCache):
	...
	def get(self, key):
		for i in range(0, len(self._client)):
	...
	def hmget(self, name, keys, *args):
		return self._primary_cache.hmget(name, keys, *args)

	def hmset(self, key, mapping):
		if not self._primary_cache.hmset(key, mapping):
			return False
		for client in self._reversed_caches:
...
def init_cache(config):
	...
	codis: host, port, unix_socket_path, password, db, default_timeout, key_prefix
...
def hmget(name, keys, *args):
	return _cache_manager.default_client.hmget(name, keys, *args)


def hmset(key, mapping):
	return _cache_manager.default_client.hmset(key, mapping)
