...
class ApiServiceBadStatus(Exception):
...
class ApiServiceTimeout(Exception):
...
class ApiServiceClient(object):
	...
	class Api(object):
		USER_GET_PROFILES = "/s2s/user/get_profiles"
	...
	def _request(self, api, data, method, app_id=None):
		self._request_id += 1
		log.info("request_api_service|host=%s,api=%s,request_id=%s,method=%s,data=%s", self._host, api, self._request_id, method, data)
		...
		if response.status_code != 200:
			log.error("request_api_service|request_id=%s,status_code=%s,api=%s", self._request_id, response.status_code, api)
			raise ApiServiceBadStatus(self._request_id, response.status_code)

		result = response.json()
		result_code = result["result"]
		result_body = result.get("reply")
		elapsed = get_timestamp_ms() - start_time
		log.info("request_api_service|elapsed=%s,request_id=%s,status_code=%s,api=%s", elapsed, self._request_id, response.status_code, api)
		...
		if disable_retry:
			...
			while True:
				...
				try:
					...
					if retry_count > 0 and result_code == ApiServiceClient.Result.ERROR_SERVER:
						log.warn("get_error_server|request_id=%s,retry_count=%s", self._request_id, retry_count)
					else:
						return result_code, result_body
				except requests.exceptions.ConnectTimeout:
					log.exception("request_api_service|request_id=%s", self._request_id)
					if retry_count == 0:
						raise ApiServiceTimeout(self._request_id)
				except ApiServiceBadStatus as error:
					log.exception("request_api_service|request_id=%s,http_code=%s", self._request_id, error.http_status)
					if retry_count == 0:
						raise
