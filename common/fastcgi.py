...
class FastcgiCommand(object):
	...
	def handle(self, *args, **options):
		...
		if options['monkey_patch']:
			names = [_f for _f in map(str.strip, options['monkey_patch'].split(',')) if _f]
			if names:
			...
			def __call__(self, fastcgi_request):
				...
				try:
					...
					try:
						...
						log.exception('handle_http_request_exception')
					except:
						pass
		...
		kwargs = dict((
			(name, value) for name, value in options.items() if name in (
