...
def ensure_str(potential_unicode_string):
	if isinstance(potential_unicode_string, str):
		potential_unicode_string = potential_unicode_string.encode('utf-8')
			...
			if isinstance(form, FormValidator):
				...
				if data_format == 'JSON':
					...
					if form is not None:
						...
						if params_errors:
							log.warning('view_params_error|format=json,url=%s,error=json_validator:%s,body=%s',
...
class _ServerSessionInterface(sessions.SessionInterface):
	...
	def open_session(self, app, request):
		...
		try:
			...
			log.exception('session_get_fail|session_key=%s', session_key)
			return self.session_class()
	...
	def save_session(self, app, session, response):
			...
			if session.modified:
				if session.session_key is not None:
					try:
						...
						log.exception('session_delete_fail|session_key=%s', session.session_key)
		...
		if session.session_key is None:
			...
			try:
				...
				log.exception('session_try_get_fail|session_key=%s', session.session_key)
				return
			if retry <= 0:
		...
		if set_cache:
			...
			try:
				...
				log.exception('session_set_fail|session_key=%s', session.session_key)
